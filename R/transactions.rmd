...
points2 <- abline(lm2$coefficients,as.Date("2017-03-25"),as.Date("2017-12-31"))
points2$X1 <- as.Date(points2$X1,origin="1970-01-01")
points3 <- abline(lm3$coefficients,as.Date("2017-01-01"),as.Date("2017-12-31"))
...
Contracts after 23/May/17 | <strong>`r round(lm2$coefficients[2],2)`</strong>  
NCCE briefs after 23/May/17 | <strong>`r round(lm3$coefficients[2],2)`</strong>  
...
max_val_3364729 <- 10  #max(hist(vals3364729,breaks=breaks)$counts)
dens3364729 <- density(vals3364729)
dens3364729$y <- dens3364729$y / max(dens3364729$y) * max_val_3364729 * 1.7
vals3413842 <- sonAll[sonAll$SON.ID=="SON3413842",]$LogValue
max_val_3413842 <- 15 #max(hist(vals3413842,breaks=breaks)$counts)
dens3413842 <- density(vals3413842)
dens3413842$y <- dens3413842$y / max(dens3413842$y) * max_val_3413842 * 1.6
```
```{r, echo=FALSE}
p <- plot_ly(alpha = 0.6) %>%
