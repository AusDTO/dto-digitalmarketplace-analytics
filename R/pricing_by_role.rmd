...
for (i in 1:nrow(frequent_roles)) {
  ...
  # identify briefs - using an edit distance approach - adist() to manage minor variations, eg spelling
  edit_dist <- data.frame(
                drop(
                  attr(
                    adist(frequent_roles$role_name[i], b$title, counts = TRUE, partial = TRUE, ignore.case = TRUE), 
                    "counts"
                    )
                  )
                ) %>%
