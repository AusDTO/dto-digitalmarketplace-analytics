...
```{r,echo=FALSE}
x <- briefs %>% group_by(type) %>% summarise(count = length(type))
...
<br />  
Monthly value of contracts = $`r monthly_value`M  
...
`r kable(by_phase,digits=2)`  
Number of briefs with phase not set: `r NAs` - these are not counted above  

  
## Trend for day rates  
```{r,echo=FALSE}
br <- briefResponses %>% filter(applicationDate <= end)
...
```
Average number of responses: `r round(mean(b_filt$responses))`  
(To all Open to All and closed briefs - **figure is not limited to the dates in the report**)  
...
#### Contracts to SMEs for this month  
`r kable(m_by_sme,digits=1)`  
